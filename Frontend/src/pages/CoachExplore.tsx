...
export default function CoachExplorePage() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      <TopBar
        ...
        onSearchClick={() => navigate("/explore-search?from=/coach-explore")}
        ...
        <div className="mx-auto max-w-2xl space-y-3">
          {visibleAthletes.length > 0 ? (
            visibleAthletes.map(a => (
              <FeedCard key={a.id} athlete={a} isFollowing={false} onToggleFollow={toggleFollow} onSendFeedback={(athlete) => setFeedbackTarget(athlete)} onProfileClick={() => openProfile({ targetRole: "athlete", targetId: a.id })} />
            ))
          ) : (
